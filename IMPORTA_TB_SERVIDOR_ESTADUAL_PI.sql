 ...
 LOOP
  ...
  IF INSTR(ENTRADA,'PI') > 0 THEN
    ...
    LINHA := DB_BASE.F_AJUSTA_CAMPO_NOVO3(LINHA, 8);
     ...
     V_MATRICULA := NULL;
     V_NOME := TRIM(F_PEGA_CAMPO(LINHA,1,';'));
     V_RENDA_BRUTA := TRIM(F_PEGA_CAMPO(LINHA,8,';'));
     V_CARGO := TRIM(F_PEGA_CAMPO(LINHA,3,';'));
     V_ORGAO := TRIM(F_PEGA_CAMPO(LINHA,4,';'));
     --V_COMPETENCIA  :='092020';
     V_CPF_PARTE := TRIM(F_PEGA_CAMPO(LINHA,2,';'));
     V_CPFINFO :='';
     V_SITUACAO := 'ATIVO';
     V_CIDADE := TRIM(F_PEGA_CAMPO(LINHA,7,';'));
     --V_UF := 'AM';
     V_ANO :='';
     V_MES :='';
     V_VINCULO := NULL;
